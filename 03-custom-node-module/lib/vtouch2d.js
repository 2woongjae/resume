/**
 * Edited by leewoongjae on 15. 11. 16..
 */
module.exports = (function(){
  'use strict';

  var dgram = require('dgram');
  var version = 'vtouch-nodejs-20151116';
  var space = {
    kinect: {
  ...
  var callback = function() {
    ...
    console.log('callback binding error');

  };

  var hit = function(_space, _callback){

    if (_space !== null && _space !== undefined) space = _space;

    if (_callback !== null && _callback !== undefined && typeof _callback === 'function') callback = _callback;
      ...
      */
      ...
      for (var i = 0; i < 6; i++) {
        ...
        if (user[0] === 'N') { // { id: 0, isTracking: false }
          ...
          // - 손가락
          if (user[10] === 'N') tmp.isFinger = false;
          else tmp.isFinger = true;

          var finger = (tmp.isFinger) ? transform(Number(user[10]), Number(user[11]), Number(user[12]), kinect) : {x:0, y:0, z:0};
  ...
  var cancel = function(str) {
    ...
    client.send(message, 0, message.length, 50415, 'localhost', function() {
  ...
  var face = function(str) {

    var message = new Buffer(str + '');
    var client = dgram.createSocket('udp4');

    client.send(message, 0, message.length, 50204, 'localhost', function() {
