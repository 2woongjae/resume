...
const CONSTANT = Constant.VodList;

class VodList extends Base {
  ...
  reinitialize = () => {
    ...
    return Promise.resolve()
                  ...
                  .then(list => {
                    ...
                    this.list.forEach((vod, index) => {

                      const image_url = vod.image;

                      vod.image = new Image();
                      vod.image.src = image_url;
                      vod.left = 300 + (330 * index);
  ...
  getList = () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      $.get('http://vtv.vtouchinc.com/api/vods', (data) => {
  ...
  Render() {
    ...
    this.list.forEach((vod, i) => {
      ...
      this.drawImage(this.context, vod.image, {cx: vod.left, cy: 505, w: 300, h: 460}, CONSTANT.LIST_SHADOW);
      this.drawRect({color: 'rgba(0, 0, 0, 0.3)', shape: {cx: vod.left, cy: 770, w: 300, h: 70}});
      this.drawText(
        this.context,
        {title: vod.title, width: vod.left - 134, height: 779, font: CONSTANT.VOD_TITLE_FONT, fillStyle: 'rgba(255, 255, 255, 1)', textAlign: 'left'},
      );
